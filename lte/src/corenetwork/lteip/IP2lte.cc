...
void IP2lte::initialize(int stage)
{
    if (stage == inet::INITSTAGE_LOCAL)
    {
        ...
        if (nodeType_ == UE)
        {
            ...
            MacNodeId macId=getBinder()->registerNode(ue, nodeType_, ue->par("masterId"));
            //HAEIN: origianl code doesn't register its name to binder, so I added below line.
            getBinder()->registerName(macId,ue->getFullName());
        }
